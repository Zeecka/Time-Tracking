...
const JS_TEMPLATE_STORAGE_KEY = 'export-js-template';

const DEFAULT_JS_TEMPLATE = `const entries = timeEntries.map(entry => ({
  user: entry.user?.name ?? '',
  project: entry.project?.name ?? '',
  tracking_code: entry.project?.tracking_code?.code ?? '',
  start_date: entry.start_date,
  end_date: entry.end_date,
  days: activeColumns.find(c => c.id === 'days')?.getValue(entry) ?? 0,
  note: entry.note ?? '',
}));

const lines = [
  '// Generated by Time Tracker',
  '// Period: W' + filters.start_week_number + '-' + filters.start_year
    + ' to W' + filters.end_week_number + '-' + filters.end_year,
  '// Entries: ' + entries.length,
  '',
  'const timeTrackingEntries = ' + JSON.stringify(entries, null, 2) + ';',
  '',
  "// Adapt the loop below to your app's registration logic",
  '(async () => {',
  '  for (const entry of timeTrackingEntries) {',
  "    console.log('Registering:', entry);",
  '    // Example: await registerEntry(entry);',
  '  }',
  '})();',
];
...
const getInitialJsTemplate = () => {
  try {
    const stored = window.localStorage.getItem(JS_TEMPLATE_STORAGE_KEY);
    if (stored !== null) return stored;
  } catch (_) { /* ignore */ }
  return process.env.REACT_APP_EXPORT_JS_TEMPLATE || DEFAULT_JS_TEMPLATE;
};
...
function ExportExcel() {
  ...
  const [jsTemplate, setJsTemplate] = useState(getInitialJsTemplate);
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(JS_TEMPLATE_STORAGE_KEY, jsTemplate);
    } catch (_) { /* ignore */ }
  }, [jsTemplate]);
  ...
  const handleResetJsTemplate = () => {
    setJsTemplate(process.env.REACT_APP_EXPORT_JS_TEMPLATE || DEFAULT_JS_TEMPLATE);
  };
  ...
  const handleExportJs = () => {
    ...
    const trimmedTemplate = jsTemplate.trim();
    if (!trimmedTemplate) {
      setError(t('export.jsTemplateEmpty'));
      return;
    }
    ...
    try {
      ...
      const templateFn = new Function('timeEntries', 'activeColumns', 'filters', 't', trimmedTemplate);
      const result = templateFn(timeEntries, activeColumns, filters, t);

      if (typeof result !== 'string') {
        setError(t('export.jsTemplateMustReturnString'));
        return;
      }
      ...
      a.href = url;
      a.download = `time_entries_${filters.start_year}_W${filters.start_week_number}_to_${filters.end_year}_W${filters.end_week_number}.js`;
      ...
      setError(t('export.jsTemplateError', { message: err.message }));
      console.error(err);
    } finally {
    ...
    <div>
      ...
      <Card className="mb-4">
        <Card.Header className="d-flex justify-content-between align-items-center">
          <h5 className="mb-0">{t('export.jsTemplateTitle')}</h5>
          <Button variant="outline-secondary" size="sm" onClick={handleResetJsTemplate}>
            <i className="fas fa-undo me-1"></i>
            {t('export.jsTemplateReset')}
          </Button>
        </Card.Header>
        <Card.Body>
          <p className="text-muted small mb-2">{t('export.jsTemplateHelp')}</p>
          <Form.Control
            as="textarea"
            value={jsTemplate}
        ...
        <Card.Header className="d-flex justify-content-between align-items-center">
          ...
          <div className="d-flex gap-2">
            <Button
              ...
              disabled={loading || generatingJs || exporting || timeEntries.length === 0}
            >
              <i className="fas fa-code me-2"></i>
              {generatingJs ? t('export.jsGenerating') : t('export.jsExport')}
            </Button>
            <Button
              variant="success"
              onClick={handleExportExcel}
              disabled={loading || exporting || generatingJs || timeEntries.length === 0}
            >
